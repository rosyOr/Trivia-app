    ...
    while restantes > 0:
        ...
        if code == 0:
            ...
            for raw in results:
                ...
                cat_es = traduccion.traducir_texto(cat_en) # <--- AQUI se traduce
                ...
                # TRADUCIR PREGUNTA
                # --------------------------------------------
                pregunta_es = traduccion.traducir_texto(src_en) # <--- AQUI SE TRADUCE
                ...
                if item["type"] == "boolean":
                    ...
                    corr_es = traduccion.traducir_texto(corr_en) # <--- AQUI SE TRADUCE
                    db.session.add(OpcionRespuesta(
                        pregunta_id=p.pregunta_id,
                        texto=corr_es,
                        es_correcta=True
                    ...
                    for inc_en in item["incorrect_answers"]:
                        inc_es = traduccion.traducir_texto(inc_en) # <--- AQUI SE TRADUCE
                        db.session.add(OpcionRespuesta(
                            pregunta_id=p.pregunta_id,
                            texto=inc_es,
                            es_correcta=False
